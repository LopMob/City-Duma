...
def _get_meeting_or_404(db: Session, meeting_id: int):
    ...
    if meeting is None:
        raise HTTPException(status_code=404, detail=f"Заседание id={meeting_id} не найдено")
    return meeting


@router.post("", response_model=schemas.MeetingRead, status_code=201)
def create_meeting(data: schemas.MeetingCreate, db: DbSession):
    if data.commission_id is not None and crud.get_commission(db, data.commission_id) is None:
        raise HTTPException(
...
def set_meeting_status(meeting_id: int, data: schemas.MeetingStatusUpdate, db: DbSession):
    meeting = _get_meeting_or_404(db, meeting_id)
    return crud.set_meeting_status(db, meeting, data.status)


@router.post("/{meeting_id}/attendance", response_model=schemas.AttendanceRead, status_code=201)
def mark_attendance(meeting_id: int, data: schemas.AttendanceCreate, db: DbSession):
    ...
    if crud.get_deputy(db, data.deputy_id) is None:
        raise HTTPException(status_code=404, detail=f"Депутат id={data.deputy_id} не найден")
